...
wait: 'Tunggu Bentar ,Senpai ^-^',
eror: 'Sepertinya Eror >~<',
success: 'Success️ OwO',
error: {
stick: 'Khusus Sticker',
Iv: 'Link Invalid!'
},
only: {
group: 'Cuman Bisa Dipake Di Grup :(',
player: 'Khusus Orang Yang Join Rpg', 
event: 'Event Belum Aktif Di Group ini!!',
owner: 'Khusus Owner ,Senpai',
admin: 'Khusus Admin ,Senpai',
Badmin: 'Jadikan Saya Admin :D', 
}
}
const isUrl = (url) => {
...
await Ryuu.setStatus(`${botname} | Ketik .menu | Bot Telah Online Selama : ${uptimer}`).catch((_) => _);
...
const daftar1 = `Hai ${pushname}\nKamu Belum Terdaftar Senpai.. , silahkan Ketik Tombol Dibawah Untuk Daftar !!`
const daftar2 = '```Klik Tombol Di Bawah Untuk Verify Kak```'
const daftar3 = [{buttonId: `${prefix}verify`,buttonText: {displayText: `✒️ DAFTAR 📄`,},type: 1,},]
...
ptod = "6283833771579@s.whatsapp.net"
stod = `${sender}`
dtod = 'ngetes'
uwu = '```'
...
*Mau Invite Saya ke Grup ? Hubungi Ownerku ya*.. ^•^

*Info User* 👤
🌱 _*Name User*_ : ${pushname}
🏷 _*Bio User*_ : ${p.status==undefined?`Not Found`:p.status}
📱 _*Nomor*_ : @${sender.split('@')[0]}`
 Sendbutdocument(from, tescok, "Script By Ryuu", fs.readFileSync('./media/Ryuu.pdf'),{mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/Ryuu.jpg'), filename:`02.pdf`,pageCount: 999 }, [{buttonId:`${prefix}listmenu`,buttonText:{displayText:'LIST MENU 📃'},type:1},{buttonId:`${prefix}allmenu`,buttonText:{displayText:'ALL MENU ⛩️'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT 🎥'},type:1}], {quoted:fvid, contextInfo: { mentionedJid: [ptod,dtod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} ${pushname}\nScript Create By Ryu`,mediaType:"2",thumbnail:fakeimage,mediaUrl:`https://youtu.be/Q6utY1yaXAY`}}})
break
case 'listmenu':
if (isBanned) return reply('Kamu Sudah Di banned!')
...
reply(`Itu Senpai.. Nomor Ownerku •^•`)
break
case 'creator':
...
*SNK*📄
1. Data Whatsapp Senpai Akan Kami Simpan Di Server , Sampai Saya Offline ya ^•^ 
2. Data Senpai Akan Di Hapus Jika saya Offline >///<
3. Saya Tidak Menyimpan Dokumen,Gambar,dsb Nya...
4. Kami Tidak Akan Pernah Meminta Senpai Untuk Memberikan Informasi Pribadi
5. Jika Menemukan BUG/ERROR Silahkan Langsung Lapor Ke OWNER Saya ya Senpai •^•


*ATTENTION*❗
*KONSEKUENSI JIKA MELANGGAR RULES >:D*
- Saya Akan Memblokir Nomor Anda.
- Senpai Tidak Akan Bisa Mengakses Saya Lagi >.<
- Mengeksploitasi Terhadap bot.
Sanksi: *Dikacangin Permanen :')*`
...
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol}) 
if (isBanned) return reply('Kamu Sudah Di banned!')
if (args.length == 0) return reply(`Example: ${prefix}${command} Jiee Botz`)
reply(mess.wait) 
anu = args.join(" ")
...
Ryuu.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Succes By : © Jiee Botz'})
break
case 'tiktokwm':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol}) 
if (isBanned) return reply('Kamu Sudah Di banned!')
